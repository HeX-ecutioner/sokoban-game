...
import com.sokoban.entity.Player;
import com.sokoban.util.Randomizer;
...
public class Grid {
    ...
    public Grid(int width, int height, int boxCount) //create a random grid with specific width,
    ...
    public void updateGrid() {
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                grid[j][i] = new Tile(Tile.GROUND);
                if (j == 0 || j == width - 1 || i == 0 || i == height - 1) {
                    grid[j][i] = new Tile(Tile.WALL, color);
                }
                for (int k = 0; k < boxCount; k++) {
                    if (destinations[k].getX() == j && destinations[k].getY() == i) {
                        grid[j][i] = new Tile(Tile.DESTINATION);
                    }
                }
                if (player.getX() == j && player.getY() == i) {
                    grid[j][i] = new Tile(Tile.PLAYER);
                }
                for (int k = 0; k < boxCount; k++) {
